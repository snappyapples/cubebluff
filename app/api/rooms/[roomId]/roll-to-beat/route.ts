...
/**
 * POST /api/rooms/[roomId]/roll-to-beat - Accept claim and roll to beat it
 * Player accepts the previous claim and rolls their own dice
 */
export async function POST(
  ...
  try {
    ...
    if (gameState.phase !== 'awaiting_response') {
      return NextResponse.json(
        { error: 'Cannot roll to beat in this phase' },
        { status: 400 }
      )
    }
    ...
    // The minimum claim stays the same (they accepted the previous claim)
    const newState = applyRoll(gameState, roll)

    // Save state with roll
    ...
    console.error('Error rolling to beat:', error)
    return NextResponse.json(
